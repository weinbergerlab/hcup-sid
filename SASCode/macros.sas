/* If test_mode is on, only first year is processed in each state. It's off by default. Don't change it here, change it in sid_time_series.sas */
%let test_mode = 0;

/* If skip_import is on, we skip the HCUP asc file -> SAS dataset loading, and assume it's already been done. It's off by default. Don't change it here, change it in sid_time_series.sas */
%let skip_import = 0;
	...
	%if not &skip_import. %then %do;
		%do year=&yearstart %to &yearend;
			...
			data sid_&state..sid_&state._&year._core;
				/* The following have inconsistent lengths between different datasets; this here is the max length, to avoid truncation */
				length dx1-dx25 $7;
				length cpt1-cpt50 $5;
				/* The following are not present in all datasets; inserting them here prevents errors later during aggregation */
				length hospstco 5;
				length hfipsstco 4;
				length zip $5;
